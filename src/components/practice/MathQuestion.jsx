...
export default function MathQuestion({ current, onCorrect, onWrong, answered }) {
  ...
  return (
    <div className="flex flex-col items-center gap-2 sm:gap-3 w-full max-w-xl">
      <div className="bg-white rounded-2xl sm:rounded-3xl p-3 sm:p-5 lg:p-7 shadow-lg text-center w-full">
        <p className="text-2xl sm:text-3xl lg:text-5xl font-bold text-slate-800 leading-snug break-words">
        ...
        {current.choices.map((choice, i) => {
          ...
          return (
            <motion.button
              ...
              className={`relative p-3 sm:p-4 lg:p-5 rounded-xl sm:rounded-2xl font-bold text-lg sm:text-xl lg:text-2xl transition-all cursor-pointer
              ...
              `}
            >
              <span className={`absolute top-1 left-2 text-[10px] sm:text-xs font-bold ${isCorrect || isWrong ? 'text-white/60' : 'text-slate-400'}`}>
                {labels[i]}
              </span>
              {choice}
            </motion.button>
