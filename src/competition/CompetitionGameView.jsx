...
import { Volume2, Trophy, Award, Star, Sparkles, AlertTriangle, Timer, CheckCircle, Loader2, Lock, ArrowRight, BookOpen, Home, RefreshCw } from 'lucide-react'
...
function BackToHomeButton({ isDark, onBack }) {
  ...
  return (
    <motion.button
      ...
      disabled={locked}
      className={`w-full max-w-md landscape:max-w-2xl mt-3 py-3 landscape:py-2 rounded-2xl text-sm landscape:text-xs font-bold relative z-10 transition-all flex items-center justify-center gap-1.5 ${
        locked
          ? isDark ? 'bg-white/3 text-slate-600 border border-white/5 cursor-not-allowed' : 'bg-slate-100/50 text-slate-300 border border-slate-200/50 cursor-not-allowed'
          : isDark ? 'bg-white/5 hover:bg-white/10 text-slate-400 border border-white/10 cursor-pointer' : 'bg-slate-100 hover:bg-slate-200 text-slate-500 border border-slate-200 cursor-pointer'
      }`}
    >
...
export default function CompetitionGameView({ engine, level, isDark = false, nextSubjectInfo, onTransition, sessionEnded, onBack }) {
  const {
    orderedQuestions,
    timeLeft,
    questionsAnswered,
  ...
  if (phase === 'completed') {
    ...
    const attempted = questionsAnswered || total
      ...
      >
        ...
        >
          <div className="flex flex-col landscape:grid landscape:grid-cols-2 gap-4 sm:gap-6 items-center landscape:items-stretch text-center landscape:text-left">
            ...
            {/* Left Column: Subject, Name, Trophy, Tier & SVG Radial Progress Meter */}
            <div className="flex flex-col items-center justify-center text-center">
              <span className={`inline-flex items-center gap-1.5 text-[10px] sm:text-xs font-black uppercase tracking-widest px-3 py-1 rounded-full mb-2 landscape:mb-1 ${
                isDark ? 'bg-indigo-500/15 text-indigo-400 border border-indigo-500/20' : 'bg-indigo-50 text-indigo-700 border border-indigo-200'
              }`}>
        ...
        {onBack && (
          <BackToHomeButton isDark={isDark} onBack={onBack} />
        )}
        ...
        <AnimatePresence>
          {submitError && (
            ...
            >
              <div className="w-full max-w-sm sm:max-w-md bg-white border-2 border-indigo-200 rounded-3xl shadow-2xl p-5 text-center relative overflow-hidden">
                <div className="absolute top-0 right-0 w-20 h-20 bg-indigo-500/5 rounded-full blur-xl" />

                <div className="w-12 h-12 rounded-2xl bg-indigo-50 border border-indigo-100 flex items-center justify-center mx-auto mb-3 shadow-inner">
                ...
                <p className="text-[11px] sm:text-xs text-slate-500 mt-1 font-medium leading-relaxed">
                  ...
                  <strong className="text-indigo-600 block mt-0.5 font-black">Please keep this page open 😊</strong>
                </p>
